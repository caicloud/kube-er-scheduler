...
type ExtendedResourceLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ExtendedResource, err error)
	// Get retrieves the ExtendedResource from the index for a given name.
	Get(name string) (*v1alpha1.ExtendedResource, error)
...
// List lists all ExtendedResources in the indexer.
func (s *extendedResourceLister) List(selector labels.Selector) (ret []*v1alpha1.ExtendedResource, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ExtendedResource))
	})
	return ret, err
}

// Get retrieves the ExtendedResource from the index for a given name.
func (s *extendedResourceLister) Get(name string) (*v1alpha1.ExtendedResource, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("extendedresource"), name)
	}
	return obj.(*v1alpha1.ExtendedResource), nil
}
